...
export function PaymentsPage() {
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-5">
      <section className="relative overflow-hidden rounded-2xl border border-cyan-300/20 bg-slate-950/70 p-5 shadow-[0_0_80px_rgba(34,211,238,.12)] backdrop-blur-xl">
        <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-cyan-300/60 to-transparent" />
        <div className="relative z-10">
          <div className="flex items-center gap-2 text-sm font-black uppercase tracking-[0.20em] text-cyan-300">
            <Lock className="h-4 w-4" />
            Payments
          </div>
          <h1 className="mt-3 text-3xl font-black tracking-tight text-cyan-50">Unlock Premium Data Pack</h1>
      ...
      <section className="rounded-xl border border-cyan-300/20 bg-slate-950/65 p-4">
        <label className="text-sm font-black text-cyan-50" htmlFor="payments-delivery-gmail">
        ...
        <div className="relative mt-3">
          ...
          <input
            ...
            className="h-11 w-full rounded-lg border border-cyan-300/15 bg-slate-950/80 pl-9 pr-3 text-sm font-semibold text-slate-100 outline-none placeholder:text-slate-500 focus:border-cyan-300/60"
          />
        </div>
      ...
      <Tabs defaultValue="upi">
        ...
        <TabsContent value="upi">
          <div className="grid gap-5 lg:grid-cols-[360px_1fr]">
            ...
            <div className="rounded-2xl border border-cyan-300/20 bg-slate-950/65 p-5">
              <div className="flex items-center gap-2 text-lg font-black text-cyan-50">
          ...
          <div className="rounded-2xl border border-cyan-300/20 bg-slate-950/65 p-5">
            ...
            <div className="mt-5 flex flex-wrap items-center gap-3">
              <Button onClick={startStripeCheckout} disabled={loading}>
                {loading ? "Creating checkout..." : `Checkout Rs ${AMOUNT_INR}`}
              </Button>
              <a href="/api/download/sample">
                <Button variant="outline"><ExternalLink className="h-4 w-4" /> Download sample first</Button>
              </a>
            </div>
